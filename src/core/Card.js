...
const showAddToCartBtn = (showAddToCartButton) => {
    return (
    showAddToCartButton && (
        <Button onClick={addToCart} variant='outlined' className='btns'>
        Add to cart
        </Button>
    )
    );
...
const showCartUpdateOptions = (cartUpdate) => {
    ...
    cartUpdate && (
        <>
        <div className='remain'><span>{product.quantity - count}</span> item is remaining based on quantity</div>
        <div className='mt-2' style={{marginTop:'20px!important'}}>
...
return (
    ...
    <Grid container spacing={2}>
        ...
        <CardM className={classes.card}>
            ...
            <span className='black-101'>₹ {product.price}</span>{showStock(product.quantity)}
                <br />
                <br />
            <Typography gutterBottom variant='h5' component='h2'>
                {product.name}
                <span className='black-91'>(
            ...
            {showCartUpdateOptions(cartUpdate)}
            <div className='moment'>{moment(product.createdAt).fromNow()}{' '}</div>
